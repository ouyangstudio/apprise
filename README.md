...
| [DingTalk](https://appriseit.com/services/dingtalk/)  | dingtalk://   | (TCP) 443   | dingtalk://token/<br />dingtalk://token/ToPhoneNo<br />dingtalk://token/ToPhoneNo1/ToPhoneNo2/ToPhoneNo1/<br/>dingtalk://app/AppKey/AppSecret/AgentId/UserID[/UserID...]
...
| DingTalk | App Mode | `dingtalk://app/AppKey/AppSecret/AgentId/UserID[/UserID...]` | Push a work notification ("工作通知") to one or more DingTalk users via a self-built enterprise application — the same channel hermes uses. `AppKey`, `AppSecret` and `AgentId` come from the app's "Credentials & Basic Info" page on [open-dev.dingtalk.com](https://open-dev.dingtalk.com/). Targets are DingTalk user IDs; an 11–14 digit mobile number is also accepted and is resolved server-side via `/topapi/v2/user/getbymobile`. access_token is cached in-process. The original custom-robot webhook mode (`dingtalk://token/...`) still works unchanged; pick whichever fits your use case. |
